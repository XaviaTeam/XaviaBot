...
const isSpam = (memberData, preTime, argsLength, msg, atmLength) => {
    ...
    if (memberData.count >= 2) {
        if (argsLength <= 20 || msgLength <= 100) {
            return 1;
        } else if (argsLength > 20 && argsLength <= 40 || msgLength > 100 && msgLength <= 200) {
            return 2;
        } else if (
            ((argsLength > 20 && argsLength <= 40 || msgLength > 100 && msgLength <= 200) && timeBetween <= _1Sec / 2) ||
            (argsLength > 40 && argsLength <= 60 || msgLength > 200 && msgLength <= 300)
        ) {
            return 3;
        } else {
            return 4;
...
const onCall = ({ message, getLang, data }) => {
    ...
    if (!data?.thread?.info?.adminIDs?.some(e => e.id == global.botID)) return;

    const { senderID, threadID } = message;
    if (!data.thread.data.hasOwnProperty("spamWarn")) data.thread.data.spamWarn = [];
    if (!data.thread.data.spamWarn.some(e => e.id == message.senderID)) data.thread.data.spamWarn.push({ id: senderID, count: [] });
    ...
    if (!global.messageCount.some(e => e.threadID == threadID)) global.messageCount.push({ threadID, members: [] });
    if (!global.messageCount.find(e => e.threadID == threadID).members.some(e => e.id == senderID))
        global.messageCount
            .find(e => e.threadID == threadID).members
            .push({ id: senderID, count: 0, countA: [], lastTime: 0, fast: 0 });
    ...
    if (message.attachments.length > 0) memberData.countA.push(message.attachments.length);
    ...
    if (memberData >= 60 || (msgNewLine >= 40 && avgLineLength <= 3)) memberData.fast += 12;
    else if (msgNewLine >= 40 || (msgNewLine >= 20 && avgLineLength <= 3)) memberData.fast += 5;
    ...
    let isMemberSpam = isSpam(memberData, preTime, message.args.length, message.body, message.attachments.length);

    if (memberData.count >= 2) memberData.count = 1;
    if (memberData.countA.length >= 2) memberData.countA = [memberData.countA[memberData.countA.length - 1]];

    memberData.lastTime = Date.now();
    memberData.fast += isMemberSpam;

    if (memberData.fast >= 12) {
        global.messageCount.find(e => e.threadID == threadID).members = global.messageCount.find(e => e.threadID == threadID).members.filter(e => e.id != senderID);
        ...
        if (spamWarn.count.length >= 3) {
            ...
            global.api.removeUserFromGroup(senderID, threadID, (err) => console.error(err));
        }
    }
}
