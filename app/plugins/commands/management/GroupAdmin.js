...
async function kick({ api, event, controllers }) {
    ...
    if (Object.keys(mentions).length == 0 && event.type != 'message_reply') {
        api.sendMessage("Please tag a user or reply to a user", threadID);
    } else {
        ...
        if (Object.keys(threadInfo).length == 0) {
            ...
            api.sendMessage("I won't kick myself lol", threadID);
        } else {
...
async function add({ api, event, args }) {
    const { threadID, messageID, senderID } = event;
    const input = args[0] || '';

    if (input == '') {
        api.sendMessage("Please input a user's uid or profile link", threadID, messageID);
    } else {
        ...
        if (!uid || isNaN(uid)) {
            api.sendMessage("Invalid profile link", threadID, messageID);
        } else {
            if (uid == botID) {
                api.sendMessage('Why would I add myself?', threadID, messageID);
            } else if (uid == senderID) {
                api.sendMessage('How can I add you to your group?', threadID, messageID);
            } else {
...
async function box({ api, event, args, controllers }) {
    const { threadID, messageID, senderID } = event;
    const { Threads } = controllers;
    const fs = libs['fs'];
    const input = args[0] ? args[0].toLowerCase() : '';
    if (input == '') {
        api.sendMessage("Please input a command", threadID, messageID);
    } else {
        switch (input) {
            case 'info':
                {
            ...
            case 'settings':
                ...
                break;
            default:
                api.sendMessage("Invalid box command", threadID, messageID);
                break;
        }
    }
    return;
...
function refresh({ api, event, controllers }) {
    ...
    api.sendMessage("Refreshing box info...", threadID, async () => {
        const threadObj = await controllers.Threads.getInfoApi(threadID) || {};
        if (Object.keys(threadObj).length == 0) {
            api.sendMessage("Can't refresh this box", threadID, messageID);
        } else {
            api.sendMessage("Refreshed", threadID, messageID);
        }
    });
