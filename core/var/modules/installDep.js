...
async function installDependencies(dependencies, pluginName, plgPath) {
    ...
    if (notInstalled.length > 0)
        logger.custom(
            `Installing dependencies for ${fileName}`,
            "LOADER",
            "\x1b[33m"
        );
    else return true;
    ...
    for (const dependency of notInstalled) {
        try {
            ...
            logger.custom(
                `Can't install dependency ${dependency}`,
                "LOADER",
                "\x1b[31m"
            );
    ...
    if (failed) {
        ...
        logger.custom(
            "Reversed dependencies installation!",
            "LOADER",
            "\x1b[32m"
        );
...
async function loadPlugins() {
    ...
    for (const plugin of plugins) {
        logger.custom(
            `Reading dependencies for ${plugin}...`,
            "LOADER",
            "\x1b[33m"
        );
        ...
        if (plugin === "commands") {
            ...
            for (const categoryFolder of categoryFolders) {
                const categoryFolderPath = resolvePath(
                    pluginPath,
                    categoryFolder
                );
                ...
                for (const file of files) {
                    ...
                    const dependenciesCommonJS =
                        getDependenciesCommonJS(filePath);
